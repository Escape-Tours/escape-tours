...
import { Search, Bed, Car, Mountain, MapPin, AlertCircle, Anchor, Compass, Loader2, Users, Minus, Plus, Sparkles, ShieldCheck, PlusCircle } from 'lucide-react';
...
const InventoryItem = memo(({ item, onDragStart, onDragEnd, onQuickAdd, draggedId, tier }: { 
  item: BuilderItem; 
  onDragStart: (e: React.DragEvent, item: BuilderItem) => void;
  onDragEnd: () => void;
  onQuickAdd: (item: BuilderItem) => void;
  ...
  return (
    <div
      ...
      onDragEnd={onDragEnd}
      className={`group relative flex items-center justify-between p-3.5 border rounded-2xl transition-all duration-300 cursor-grab active:cursor-grabbing bg-slate-900/40 backdrop-blur-md select-none
        ${draggedId === item.id 
    ...
    >
      ...
      <div className="flex items-center gap-2.5 flex-shrink-0 relative z-10 pl-2">
        <span className="text-xs font-black text-amber-400 bg-amber-500/10 px-2.5 py-1.5 rounded-xl border border-amber-500/25 shadow-sm">
        ...
        <button
          ...
          className="p-2 rounded-xl bg-amber-400/10 border border-amber-400/30 text-amber-400 hover:bg-amber-400 hover:text-slate-950 transition-all duration-200 cursor-pointer active:scale-95"
        >
          <PlusCircle size={16} />
        </button>
      </div>
...
export const ItineraryBuilder = ({ tier: propTier, residencyTier: propResidencyTier }: ItineraryBuilderProps) => {
  ...
  const handleQuickAdd = useCallback((item: BuilderItem) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950/95 backdrop-blur-2xl rounded-3xl border border-slate-800/80 shadow-[0_20px_50px_rgba(0,0,0,0.6)] overflow-hidden text-slate-100">
      ...
      {/* Header Panel */}
      <div className="p-4 sm:p-5 border-b border-slate-800/80 space-y-3.5 bg-slate-900/80 sticky top-0 z-20 backdrop-blur-xl">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-950 rounded-xl border border-slate-800/80 shadow-inner">
            ...
            <span className="text-[9px] font-black uppercase tracking-wider text-amber-300">
              {loadingTier ? 'SYNCING...' : `${normalizedTier} TIER`}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
           <div className="flex items-center justify-between px-3 py-2 bg-slate-950/80 rounded-2xl border border-slate-800/80 shadow-sm">
        ...
        {/* Category Navigation Pills (Horizontally Scrollable on Mobile) */}
        <div className="flex sm:grid sm:grid-cols-3 gap-2 overflow-x-auto pb-1 custom-scrollbar">
          {CATEGORY_CONFIG.map((cat) => {
            ...
            return (
              <button 
                ...
                className={`flex flex-col items-center gap-1 p-2.5 min-w-[72px] sm:min-w-0 rounded-2xl transition-all duration-300 border cursor-pointer shrink-0 ${
                  isActive 
      ...
      <div className="p-4 sm:p-5 bg-slate-900/95 border-t border-slate-800/80 backdrop-blur-xl shadow-2xl relative space-y-3">
