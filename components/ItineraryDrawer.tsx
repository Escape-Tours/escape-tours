...
export const ItineraryDrawer = () => {
  const { items, tier, setTier, removeItem, totalPrice } = useItineraryStore();
    ...
    <aside className="p-6 border-l shadow-xl bg-white h-full transition-all duration-300 flex flex-col">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="relative group">
          <button className="flex items-center gap-2 text-xs font-semibold px-3 py-1.5 bg-slate-100 hover:bg-slate-200 rounded-full transition-colors">
            <Settings2 size={12} />
            {RESIDENCY_TIER[tier]}
          </button>
          ...
          <div className="absolute right-0 top-full mt-2 w-40 bg-white border rounded-lg shadow-xl p-1 hidden group-hover:block z-50">
            {(['INTERNATIONAL', 'RESIDENT', 'CITIZEN'] as ResidencyTier[]).map((t) => (
              <button
                key={t}
                onClick={() => setTier(t)}
                className={`w-full text-left px-4 py-2 text-xs rounded-md ${tier === t ? 'bg-blue-50 text-blue-700' : 'hover:bg-slate-50'}`}
              >
                {RESIDENCY_TIER[t]}
              </button>
            ))}
